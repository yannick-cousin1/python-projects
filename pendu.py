...
def list_to_string(LIST):   #function to convert a list to a string (each value of the list is concatenate into the string)
    string = ''
    for i in LIST:
        string += i
    return string
...
with open("dico_france.txt", encoding = "ISO-8859-1") as dico:   #We need to replace each special char by a normal char
    string = myLower(dico.read())                               # I've replaced all "œ" char by "oe" before
...
with open("dico_france.txt", "w") as dic:
    dic.write(string)                               # Writes the changes we've just done

############################################################################

play_again = "y"                        # We'll use this variable to keep running the game as long as the user wants
while play_again == "y":
    ...
    lvl = 0                       # lvl to choose difficulty
    ...
    while nb_lettres_trouve != length(word)-1 and hp > 0:    # While the whole word isn't found, while user still has
        ...
        print ("%d lettres à trouver pour gagner !" % (length(word)-nb_lettres_trouve-1)) # print how many letters left to win
        ...
        lettre = myLower(input("Quelle lettre proposes-tu ? : "))    # user is prompted to put a letter
    ...
    if nb_lettres_trouve == length(word)-1:                 # Two cases if we are out of the main loop :
    ...
    play_again = input("Rejouer ? [y/n] ")                   # Ask the user if he want to play again
